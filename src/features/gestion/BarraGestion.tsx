...
function EtiquetasInline({ clave }: { clave: string }) {
  ...
  return (
    <span className="flex items-center gap-1">
      ...
      {lista.map((t) => (
        <span key={t} className="group/tag inline-flex items-center gap-0.5 rounded-md border border-border bg-card px-1.5 py-0.5 text-[10.5px] text-muted-foreground">
          {t}
      ...
      {abierto ? (
        <input
          ...
          className="w-20 rounded-md border border-primary bg-card px-1.5 py-0.5 text-[10.5px] outline-none"
        />
      ) : (
        <button
          type="button"
          ...
          <div className="flex flex-wrap gap-1.5">
            {opcionesRapidas().map((o) => (
              <button
                ...
                className="rounded-full border border-border px-2 py-1 text-[10.5px] font-semibold text-muted-foreground transition-colors hover:border-primary hover:bg-secondary/40 hover:text-foreground disabled:opacity-50"
              >
                {crear.isPending ? <Loader2 size={10} className="inline animate-spin" /> : o.etiqueta}
              </button>
            ))}
          </div>
          <p className="mt-1.5 text-[10px] text-muted-foreground">Cae en tu Agenda. Nada se envía solo.</p>
        </div>
      )}
    </span>
  );
...
export function BarraGestion({ conversacion }: { conversacion: Conversacion }) {
  ...
  return (
    <div className="shrink-0 rounded-2xl border border-border bg-card px-3 py-2 shadow-panel">
      ...
      {error && (
        <p className="mt-1.5 rounded-lg bg-gold/10 px-2 py-1 text-[10.5px] font-medium text-gold-ink">{error}</p>
      )}
    </div>
  );
}
