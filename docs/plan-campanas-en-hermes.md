...
> ⚠️ **Al 16-ago-2026:** el frente se construyó, con otros nombres y otro reparto de fases. Lo que
> existe: `server/src/campana/` (`publico.ts`, `vetoAlSalir.ts`, `autorizar.ts`, `metaPlantillas.ts`,
> `lista.ts`…), `server/src/routes/campana.ts`, la solapa «Campañas» de
> `src/features/vistas/VistaPersonas.tsx` sobre `src/features/campana/`, y los ✓✓ como
> `server/src/entrega/`. Lo que NO se construyó: el despachador con techos propios, la ocupación del
> número y el chip de la cabecera. Las decisiones firmadas son
> `docs/adr/0036-campanas-por-plantilla-aprobada.md` y, para la lista de afuera,
> `docs/adr/0055-la-campana-a-una-lista-de-afuera.md`.
...
**Dónde:** Server: server/src/campana/ (hoy un solo archivo) + server/src/routes/campana.ts + una lectura de ocupación del número que este plan proponía como un `ritmo/ocupacion.ts` y que no se construyó. Front: tercera solapa «Campañas» en la vista Contactos — src/features/vistas/VistaPersonas.tsx:126, el `modo: 'padron' | 'telefono'` pasa a `| 'campanas'`, y el componente Solapa ya vive ahí (:148-167) — más un chip en la cabecera (src/App.tsx:508, al lado de InterruptorAutoRespuesta), en src/features/campana/. NO se toca VISTAS (src/App.tsx:82-98), no se gasta el noveno atajo, no hay tecla suelta nueva, no hay listener de Escape nuevo, y src/App.test.tsx:104-117 no rompe.

## Por qué

El punto de partida no es «¿construimos campañas?» sino «ya salieron 88 y el sistema que las mandó rompe una regla que juramos». server/src/scripts/campana.ts está en main desde el 4-ago, sin ADR, sin un solo test, y le pasa a huboRechazo ÚNICAMENTE primerMensaje (:229-232) cuando la firma acepta lista y su doc dice «vale para toda la conversación». Quien dijo que no en el quinto mensaje hoy recibe promoción. Eso no espera una pantalla.
...
- server/src/campana/publico.ts — PURO, con reloj inyectado y tests. Saca los cuatro .filter() de adentro de main() (server/src/scripts/campana.ts:229-265) y devuelve {elegibles, descartados: [{motivo, quienes}]}. Motivos: rechazo · despedida · ya_compro · sin_anuncio · ya_le_llego. huboRechazo recibe TODOS los mensajes; esDespedida mira el último. `ya_compro` cruza clientes_padron (el EXISTS sobre icarus.sales, nunca n_purchases).
- Un `candidatos.ts` con el SQL de traerCandidatos() y `base` inyectado (patrón ADR 0008), sin un CASE. Esa extracción no se hizo: al 16-ago-2026 `traerCandidatos()` sigue viviendo adentro de server/src/scripts/campana.ts, contra el singleton `db`. Arregla la ventana de 24 h: se calcula POR LÍNEA (hoy ultimo_entrante_global cruza números propios). Dedup con test: (telefono, pieza_clase='hsm', pieza_ref sin prefijo, estado='enviado').
...
- La guarda que plantillasAprobadas.ts:28 promete y no existe. Falla cerrado. Comparación normalizada (CRLF, espacios de cola) para no bloquear por una diferencia cosmética. Lo que este plan llamaba `verificarContraMeta.ts` se construyó como server/src/campana/metaPlantillas.ts (le pregunta el catálogo a Meta, que es quien lo sabe) + server/src/campana/estadoDePlantilla.ts (el veredicto puro, con lo desconocido cayendo siempre en «no se puede mandar»).
...
- server/src/scripts/campana.ts adelgazado: deja de leer process.argv en el tope del módulo y de usar el singleton db; consume candidatos.ts + publico.ts + vetoAlSalir.ts. Asigna con asignarSiEstaLibre (no reasignar, que pisa) y verifica el destino.
...
- Un módulo `ritmo/ocupacion.ts` —ocupacionDelNumero(base, numero, desde, hasta) leída de envios_wa— que este plan proponía y no se construyó. Esto NO es cosmético: autorespuesta/repositorio.ts:336 lee solo auto_respuestas_pendientes, y quien de verdad comparte 51984429504 todo el día es el BOT (vendedora_id='bot'). Un techo que no ve al bot no protege el canal.
- Un `config.ts` propio de campañas —CAMPANA_TECHO_HORA / CAMPANA_TECHO_DIA / ventana, propios y con tope duro— que este plan proponía y no se construyó: al 16-ago-2026 el único ritmo es el flag `--espera` (60 s por default) de server/src/scripts/campana.ts, sin techo por hora ni por día (nunca Infinity, que es lo que hoy vale --tope). NO se hereda techoPorDia: 60 de autorespuesta/config.ts: ese 60 está derivado de 150 minutos nocturnos y con 88 destinatarios deja 28 afuera con un motivo que dice «auto-respuestas».
- Un `despachador.ts` de campañas que este plan proponía y no se construyó (el bucle de envío sigue adentro de server/src/scripts/campana.ts, con el freno por error en server/src/campana/reintento.ts) — el molde de autorespuesta/despachador.ts (cerrojo enVuelo, una por tick, freno total con motivo, no vuelve solo) PERO con política propia: NO cancela la cola al empezar el horario de atención (despachador.ts:140 lo hace, y calcado la campaña se autodestruye a las 9:00). Llama programar() —que ya recibe `ocupadas` y `ventanas`— y le pasa la ocupación de envios_wa.
...
- Un `ChipCampana` + `estadoCampana` (puro, con test) montado en src/App.tsx:508. Sin oro (acá no se acaba nada); rojo con el motivo si se frenó. Solo se dibuja con campaña corriendo — el chip apareciendo ES el aviso. Galería en /galeria-campana.html. **Ese chip no se construyó**: el freno sí existe —server/src/campana/autorizar.ts, `POST /api/campana/corridas/:id/frenar` en server/src/routes/campana.ts, y los hooks `useCorridasVivas`/`useFrenar` de src/features/campana/plantillas.ts— pero ningún componente los consume todavía, así que hoy no hay aviso en la cabecera.
...
- Lectura pura: el estado más avanzado por mensaje, y null ≠ false («no lo sabemos» no se dibuja como «no llegó»). Lo que este plan llamaba `estadosEntrega.ts` se construyó como server/src/entrega/dominio.ts (la escala monótona, pura) + server/src/entrega/repositorio.ts (el UPDATE que lleva su propio WHERE, para que arbitre la base), y `value.statuses` sí se lee hoy en server/src/webhook/whatsapp.ts. La forma quedó distinta de la propuesta: no hay tabla nueva, el estado vive en columnas de `envios_wa` (migración 0021, expand-only).
...
- Un `mediaCloudApi` con la subida del flyer extraída y con UNA versión de Graph pinneada (hoy el script va a v23.0 y el transporte a v25.0, en el mismo flujo). No se extrajo: al 16-ago-2026 siguen siendo dos subidas, la del script en server/src/scripts/campana.ts y la del transporte en server/src/whatsapp/transporteCloudApi.ts.
- src/features/campana/: PantallaCampanas.tsx (los tres pasos) · una tabla de simulacro · una pantalla de autorizar · una ficha de la corrida · campana.ts (cliente + lecturas puras con test; un motivo desconocido cae en una lectura conservadora, nunca en un throw) · galeria.tsx con ?simulacro=1 ?confirmar=1 ?despachando=1 ?frenada=1. Se construyó con otro reparto de componentes: los tres pasos y el autorizar viven juntos en src/features/campana/ArmarCampana.tsx, la ficha de la corrida es src/features/campana/PantallaCorridas.tsx, y hay dos pantallas que este plan no previó — src/features/campana/PantallaListas.tsx y src/features/campana/PantallaPlantillas.tsx.
