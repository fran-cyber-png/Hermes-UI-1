...
campaignsRouter.get("/ad-accounts", async (_req, res) => {
  ...
  try {
    ...
    res.status(500).json({ type: "api_error", message: (err as Error).message });
  }
});
  ...
  const settled = await Promise.all(
    ids.map(async (accountId) => {
      ...
      try {
        ...
        return { accountId, campaigns: [], error: message };
      }
    }),
  );
...
campaignsRouter.get("/insights", async (req, res) => {
  const token = process.env.META_ACCESS_TOKEN;
  if (!token) {
    res.status(500).json({ type: "config_error", message: "META_ACCESS_TOKEN no está configurado en el servidor." });
    return;
  ...
  const settled = await Promise.all(
    ids.map(async (accountId) => {
      ...
      try {
        ...
        const message = err instanceof MetaGraphError ? err.message : (err as Error).message;
        return { accountId, insights: [], error: message };
      }
    }),
  );
...
async function createOneCampaign(
  ...
  try {
    ...
    return { accountId, type: "api_error", message: (err as Error).message };
  }
}
