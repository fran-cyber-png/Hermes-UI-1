 ...
 * ══ POR QUÉ EXISTE ═══════════════════════════════════════════════════════════
 *
 * La frontera de la cola deja de ser opt-in (D4): a partir del deploy que la
 * lleve, **toda persona que no sea supervisora ve sólo lo suyo más lo huérfano
 * de su alcance de línea**. Eso es un cambio de lo que las vendedoras ven en la
 * primera pantalla de su día, y falla de dos maneras opuestas:
 *
 ...
 * dueña. Por eso las identidades salen de la UNIÓN de tres tablas
 * (`identidadesActivas`), no de una.
 *
 * ══ CÓMO SE USA ══════════════════════════════════════════════════════════════
 *
 ...
 */
async function medir(vendedoraId: string): Promise<FilaPreflight> {
  const conFrontera = await consultarCola(db, { vendedoraId, limit: 1 });
  const mias = await consultarCola(db, { vendedoraId, misAsignadas: true, limit: 1 });
  const total = conFrontera.total ?? 0;
  const propias = mias.total ?? 0;
  return {
    vendedoraId,
    esSupervisora: esSupervisor(vendedoraId, process.env),
    total,
    propias,
...
async function main() {
  ...
  const supervisores = supervisoresConfigurados(process.env);
  console.log("── PREFLIGHT DE LA FRONTERA DE LA COLA ──────────────────────────");
  console.log(`Base            : ${(process.env.DATABASE_URL ?? "(sin DATABASE_URL)").replace(/:\/\/[^@]*@/, "://***@")}`);
  console.log(`Supervisoras    : ${supervisores.length ? supervisores.join(", ") : "(ninguna — fail-closed: TODAS quedan recortadas)"}`);
  ...
  for (const { vendedoraId, origenes } of identidades) {
    const fila = await medir(vendedoraId);
    filas.push(fila);
    const alcance = await leerAlcance(db, vendedoraId);
    const rol = fila.esSupervisora ? "supervisora" : "vendedora";
    console.log(
      `${vendedoraId.padEnd(28)} ${rol.padEnd(12)} ve ${String(fila.total).padStart(6)}` +
